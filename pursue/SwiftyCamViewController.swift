...
open class SwiftyCamViewController: UIViewController {
    ...
    fileprivate func addVideoInput() {
        ...
        do {
            guard let recordingDevice = videoDevice else { return }
            let videoDeviceInput = try AVCaptureDeviceInput(device: recordingDevice)
...
extension SwiftyCamViewController {
    ...
    @objc private func panGesture(pan: UIPanGestureRecognizer) {
        ...
        do {
            guard let captureDevice = AVCaptureDevice.devices().first else { return }
            try captureDevice.lockForConfiguration()
